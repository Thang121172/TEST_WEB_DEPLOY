...
from menus.models import Merchant, MerchantMember  # sẽ được định nghĩa trong app menus
...
# =========================================================
# 2. Đăng ký Merchant (tạo user nếu chưa đăng nhập)
#    / hoặc convert user hiện tại thành merchant
# =========================================================
class RegisterMerchantSerializer(serializers.Serializer):
    ...
    Merchant model và MerchantMember sẽ tồn tại trong menus.models,
    và MerchantMember(role="owner") gắn user với merchant vừa tạo.
    """
...
#
# Bước 1: /api/accounts/register/request-otp/
#   body: { "email": "...", "password": "...", "role": "customer" }
#   -> Tạo OTPRequest + gửi email
#
# Bước 2: /api/accounts/register/confirm/
#   body: { "email": "...", "otp": "123456" }
#   -> Kiểm tra OTP, tạo User + Profile, trả JWT
# =========================================================

class RegisterRequestOTPSerializer(serializers.Serializer):
    """
    Người dùng nhập email + password + role để xin OTP.
    Chúng ta tạo OTPRequest (expires_at = now + 5 phút)
    và sẽ trigger task gửi email OTP (Celery).
    """
...
#    Bước A: xin OTP reset (dùng RegisterRequestOTPSerializer logic nhưng purpose khác -> view lo)
#    Bước B: xác nhận OTP + mật khẩu mới
# =========================================================
